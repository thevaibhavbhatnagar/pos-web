...
import PosComponent from './_components';
...
export default async function Page({ searchParams }: { searchParams: Promise<{ page?: string; search?: string; sort_by?: string; sort_dir?: string; category_id?: string }> }) {
  ...
  const [productResponse, categoryResponse] = await Promise.all([
    axiosInstance.get(apiEndpoints.product.getProductsByCategory(params.category_id || ""), { params: { page, limit } }),
  ...
  return (
    <PosComponent products={products} categories={categories} statuses={statuses} page={page} totalPages={totalPages} rowsPerPage={limit} />
  )
}
